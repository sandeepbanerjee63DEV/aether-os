...
const OP_STATUS_META: Record<string, { tone: string; label: string }> = {
  ...
  CONTACTED: { tone: "bg-blue-100 text-blue-700", label: "Contacted" },
  ENGAGED: { tone: "bg-emerald-100 text-emerald-700", label: "Engaged" },
  STALE: { tone: "bg-amber-100 text-amber-700", label: "Stale" },
...
export function LeadOwnerCard({
  ...
  const suggestionsQuery = useQuery<OwnerSuggestionsResponse>({
    queryKey: ["lead-owner-suggestions", leadId],
    queryFn: async () => {
      const res = await fetch(`/api/leads/${leadId}/owner-suggestions`);
  ...
  const reassignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["lead-owner-suggestions", leadId] });
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              No owner yet. Let the AI Engine pick one based on current workforce signals.
            </p>
          </div>
          <Button
            size="sm"
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
                  ...
                  >
                    <div className="border-b border-slate-100 p-3">
                      ...
                      <p className="mt-0.5 text-[11px] text-slate-500">
                        Based on current workforce signals
                      </p>
                    </div>
          ...
          {followUpOwner && followUpOwner.id !== owner.id && (
            <div className="mt-2 flex items-center gap-1.5 rounded-lg border border-amber-100 bg-amber-50/60 px-2 py-1 text-[10px] text-amber-800">
              <CheckCircle2 className="h-3 w-3" />
              Follow-up owner: <span className="font-semibold">{followUpOwner.name}</span>
            </div>
          )}
        </div>
      </div>
